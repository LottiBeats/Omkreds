...
export default function SteelBeamBlock({ block, onChange, blocks = [] }) {
  ...
  // For beam_fem: _summary.M_Ed_kNm; for general_frame_fem: pick element from _exports
        ...
        <Field label="FEM block" style={{ gridColumn: '1/-1' }}>
          {femBlocks.length === 0
            ...
            : <div style={{ display:'flex', gap:10, alignItems:'center', flexWrap:'wrap' }}>
                ...
                {/* Element picker for general_frame_fem */}
                {isGenFem && genExports.length > 0 && (<>
                  <select style={{ ...inp, maxWidth: 200 }}
                    value={selElemId ?? ''}
                ...
                {femReady ? (() => { const {M,V} = getFemMV(); return (
                  <span style={ok}>✓ M_Ed = {M?.toFixed(2)} kNm · V_Ed = {V?.toFixed(2)} kN</span>
                )})() : isGenFem
                  ? <span style={warn}>Run the Frame FEM block first</span>
